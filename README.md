...
demo-kb だと次のように出る（抜粋）。

```
## 議題

### AGD-001 図面PDFを検索対象に入れるかを決めたい
- 提起者: 顧客担当者（2026-08-21）
- 状態: 継続（持ち越し）
#### これまでの決定
- DEC-012 図面PDF（スキャンPDFを含む）を検索対象に追加する（2026-08-28・顧客PM）
#### 未決の問い
##### Q-013 図面PDF対応の追加費用をどう扱うか
#### 未完了のアクション
- ACT-011 図面PDF対応の追加見積を提示する
```
...
- 決着したかは確認②で人が決める（`kime review` の 2-5）。決めなければ次回に残る
